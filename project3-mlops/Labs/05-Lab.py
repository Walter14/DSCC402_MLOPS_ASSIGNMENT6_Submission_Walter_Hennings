...
X_train_processed["trunc_lat"] = X_train['latitude'].apply(lambda x: round(x,2))
X_train_processed["trunc_long"] = X_train['longitude'].apply(lambda x: round(x,2))
X_train_processed["review_scores_sum"] = X_train[["review_scores_cleanliness","review_scores_checkin","review_scores_communication","review_scores_location","review_scores_value"]].sum(axis = 1)
X_train_processed = X_train_processed.drop(["latitude","longitude","review_scores_cleanliness","review_scores_checkin","review_scores_communication","review_scores_location","review_scores_value"], axis=1)

# pre-process test data to obtain MSE
X_test_processed = X_test.copy()
X_test_processed["trunc_lat"] = X_test['latitude'].apply(lambda x: round(x,2))
X_test_processed["trunc_long"] = X_test['longitude'].apply(lambda x: round(x,2))
X_test_processed["review_scores_sum"] = X_test[["review_scores_cleanliness","review_scores_checkin","review_scores_communication","review_scores_location","review_scores_value"]].sum(axis = 1)
X_test_processed = X_test_processed.drop(["latitude","longitude","review_scores_cleanliness","review_scores_checkin","review_scores_communication","review_scores_location","review_scores_value"], axis=1)
...
class RF_with_preprocess(mlflow.pyfunc.PythonModel):
    ...
    def preprocess_input(self, model_input):
        ...
        model_input["trunc_lat"] = model_input['latitude'].apply(lambda x: round(x,2))
        model_input["trunc_long"] = model_input['longitude'].apply(lambda x: round(x,2))
        model_input["review_scores_sum"] = model_input[["review_scores_cleanliness","review_scores_checkin","review_scores_communication","review_scores_location","review_scores_value"]].sum(axis = 1)
        model_input = model_input.drop(["latitude","longitude","review_scores_cleanliness","review_scores_checkin","review_scores_communication","review_scores_location","review_scores_value"], axis=1)
        return model_input
...
workingDir = "dbfs:/user/whenning@ur.rochester.edu/mlflow/05_lab2"
...
class RF_with_postprocess(mlflow.pyfunc.PythonModel):
    ...
    def categorizer(self, price_value):
        categorical_value = ''
        if price_value > 100:
            categorical_value = "Expensive"
        else:
            categorical_value = "Not Expensive"
    ...
    def postprocess_result(self, results):
        ...
        for i in range(len(results)):
            if results[i] > 100:
                cat_results.append("Expensive")
            else:
                cat_results.append("Not Expensive")
        return cat_results
...
model_path =  f"{workingDir}/RF_with_postprocess/10"

dbutils.fs.rm('dbfs:/user/whenning@ur.rochester.edu/mlflow/05_lab/RF_with_postprocess', True) # remove folder if already exists
