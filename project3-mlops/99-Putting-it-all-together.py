...
# TODO  
#This is a Pandas DF, not a Spark DF. Use Pandas functions and regex for $ signs.
...
temp = pd.DataFrame()
for i in airbnbDF.columns:
    
    if airbnbDF[i].dtype != "float64":
        temp[i] = airbnbDF[i]
display(temp)
#I want to eliminate neighborhood_cleansed, and keep others.
temp = temp.drop(["neighbourhood_cleansed","zipcode"],axis = 1)
airbnbDF = airbnbDF.drop("neighbourhood_cleansed", axis = 1)
airbnbDF.zipcode = airbnbDF.zipcode.astype("float")
...
airbnbDF_encoded = airbnbDF
for feature in temp.columns:
    airbnbDF_encoded[feature] = ordinal_encoder.fit_transform(airbnbDF[[feature]])
display(airbnbDF_encoded)
#Using Ordinal Encoding instead of one hot encoding. But that could work too probably
...
X = airbnbDF_encoded.drop("price", axis = 1)
y = airbnbDF_encoded.price
...
#fit = lassoreg.fit(X_train,y_train)
#y_pred = fit.predict(X_test)
#y_pred
...
with mlflow.start_run():
    mlflow.sklearn.log_model(linreg, "linear-regression-model")
    mlflow.log_metric("mse", mse)
    mlflow.log_param("x", 1)
#with mlflow.start_run():
#    mlflow.sklearn.log_model(lassoreg, "lasso-regression-model")
#    mlflow.log_metric("mse", mse)
#    mlflow.log_param("alpha", 1)
    
#with mlflow.start_run():
#    mlflow.sklearn.log_model(ridgereg, "ridge-regression-model")
#    mlflow.log_metric("mse", mse)
#    mlflow.log_param("alpha", 1)
...
experimentID = 3805545008150382

client = MlflowClient()
#

runs = client.search_runs(experimentID, order_by=["metrics.acc asc"])
#for i in range(len(runs)):
runs[0].data.metrics
arifactURI = 'runs:/'+runs[0].info.run_id+"/linear-regression-model" 

model = mlflow.pyfunc.load_model(arifactURI) #The linear model has the best performance based on lowest MSE
...
class Airbnb_Model(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input):
        # FILL_IN
        results = self.linreg.predict(model_input)
        postprocess_result = results/self.column
...
final_model_path =  f"{working_path}/final-model2"
post_process_model = Airbnb_Model(model = linreg, column = X_test['accommodates'])

dbutils.fs.rm(final_model_path, True)

mlflow.pyfunc.save_model(path=final_model_path.replace("dbfs:", "/dbfs"), python_model=post_process_model)
...
def model_predict(final_model_path, test_data_path, prediction_path):
    test_data = pd.read_csv(test_data_path) #Read data
    loaded_model = mlflow.pyfunc.load_model(final_model_path) #load model
    model_output_priceperperson = loaded_model.predict(test_data) #Get predictions
    model_output_priceperperson.to_csv(prediction_path, index = False) #Save predictions
    return model_output_priceperperson
...
def model_predict(final_model_path, test_data_path, prediction_path):
    ...
    loaded_model = mlflow.pyfunc.load_model(final_model_path) 
    model_output_priceperperson = loaded_model.predict(test_data) 
    model_output_priceperperson.to_csv(prediction_path, index = False) 
    return model_output_priceperperson
